...
public class Kata {
	public static String HighAndLow(String numbers) {
		...
		int high = Integer.parseInt(strings[0]);
		;
		for (int i = 0; i < strings.length; i++) {

			if (low > Integer.parseInt(strings[i])) {
				low = Integer.parseInt(strings[i]);
				// System.out.println("low =" + low);
			}
			if (high < Integer.parseInt(strings[i])) {
				high = Integer.parseInt(strings[i]);
